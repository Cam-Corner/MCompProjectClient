...
ATP_ThirdPersonCharacter::ATP_ThirdPersonCharacter()
{
	...
	_PlayerDisplayName = CreateDefaultSubobject<UWidgetComponent>(TEXT("Display Name"));
	_PlayerDisplayName->SetupAttachment(RootComponent);
}
...
FString ATP_ThirdPersonCharacter::GetCorrectDisplayName()
{
	APlayerController* PC = UGameplayStatics::GetPlayerController(GetWorld(), 0);
	AMPGameState* GS = Cast<AMPGameState>(UGameplayStatics::GetGameState(GetWorld()));

	if (PC != NULL)
	{
		AMPPlayerState* PS = Cast<AMPPlayerState>(PC->PlayerState);
		if (PS != NULL)
		{
			if (GetOwner() == PC)
			{
